    ...
    for i in df.index:
        ...
        if sub <= 5:
            TeacherModel.objects.create(FirstName=FirstName, LastName=LastName, Profilepicture=Profilepicture,
                                        EmailAddress=EmailAddress, PhoneNumber=PhoneNumber, RoomNumber=RoomNumber, Subjectstaught=Subjectstaught)
...
def FilterTeacherProfile(request):
    ...
    if request.method == 'POST':
        ...
        data = TeacherModel.objects.filter(
            Q(LastName__icontains=filtertxt) | Q(Subjectstaught__icontains=filtertxt))
        return render(request, 'FilterForm.html', {'data': data})
...
def AddTeacherData(request):
    if request.method == 'POST':
        ...
        TeacherModel.objects.create(FirstName=firstName, LastName=lastName, EmailAddress=email,
                                    PhoneNumber=mobile, RoomNumber=roomnumber, Subjectstaught=subjects, Profilepicture=profilepic)
        qs = TeacherModel.objects.all()
        return render(request, 'AllData.html', {'data': qs})
